...
class knn_regressor:
    """
    K-Nearest Neighbour Regressor, with default k=1.
    
    Can specify heuristic for 'distance'.

    1. Initialise as `knr = knn_regressor([n_neighbors:int?])`.
    2. Then use `knr.fit(X, y)`
    3. Then use `knr.predict(Xi, [heuristic?])`
    """
    ...
    def fit(self, X: np.ndarray, y: np.ndarray):
        ...
        assert isinstance(y[0], int|float) # limitation on data - classes must be int | float
    ...
    def predict(self, samples:np.ndarray, heuristic = euclidean_distance):
        ...
        for sample in samples:
            ...
            for neighbor in neighbors_focused:
                sum += neighbor['class']
            results.append(sum/self.n_neighbors)
    ...
    def score(self, samples: np.ndarray, targets: np.ndarray):
        ...
        Returns:
            float: The average relative error in this test data. (Lesser is better)
        """
